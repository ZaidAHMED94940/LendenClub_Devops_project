...
const TodoList = () => {
  ...
  const createTask = async (task) => {
    try {
      const response = await fetch(apiUrl, {
        method: "POST",
        headers: {
          "Content-Type": "application/json", // Add this to specify JSON payload
        },
        body: JSON.stringify(task), // Ensure the task object is stringified
      });
  ...
  const handleTaskDelete = async (id) => {
    ...
    if (userConfirmed) {
      try {
        // Add headers, including Content-Type and Authorization (if needed)
        const token = localStorage.getItem("authToken"); // Example for getting JWT token from localStorage
